...
from PyQt5.QtWidgets import QMainWindow, QWidget, QAction, qApp, QApplication, QDesktopWidget, QFrame, QTreeWidget, \
...
class MainWindow(QMainWindow):
    ...
    def openFile(self):
        ...
        pattern_other = re.compile(r'.*(\.raw)$')
    ...
    def saveFile(self,image,message_dic):
        if len(image.shape)>3:
            bsqImage = np.zeros((image.shape[2], image.shape[0],image.shape[1]),dtype="uint16") #转变为bsq
        ...
        if dir_path[0][-4:] == '.raw':
            ...
            if not message_dic.keys().__contains__("central_wavelength"):
                ...
                h13 = 'wavelength units = {'+message_dic["central_wavelength"]+'}'
    ...
    def openMenu(self):
        ...
        if len(items) == 1 and items[0].text(0) == "1级产品":
            action1 = menu.addAction("自动拼接")
            action1.triggered.connect(self.autoJoint_01)
            # action = menu.addAction('rename')
            # action.triggered.connect(...)
            # action = menu.addAction('delete')
            # action.triggered.connect(...)
        ...
        if len(items) == 1 and items[0].text(0) == "2A级产品":
            action1 = menu.addAction("自动拼接")
            action1.triggered.connect(self.autoJoint_2A)
            menu.exec_(QCursor.pos())

        if len(items) == 1 and items[0].text(0) == "2B级产品":
            action1 = menu.addAction("自动拼接")
            action1.triggered.connect(self.autoJoint_2B)
            menu.exec_(QCursor.pos())
    ...
    def autoJoint_01(self):  # 对01文件夹下的所有文件进行处理输出展示
    ...
    def joint_01(self,path): # 对01单个文件的处理输出展示
        try:
            self.joint_01Thread = Joint_01_Thread(path)
            self.processDiaog = ProcessDialog()
            self.joint_01Thread.start()
            self.joint_01Thread.finish.connect(self.addTable)
            self.processDiaog.exec_()
            self.joint_01Thread.wait() #阻塞进程等待主线程完成
            if self.joint_01Thread.exitcode == 1:
                raise self.joint_01Thread.exception
        except Exception as e:
            QMessageBox.information(self, "提示：", '操作失败!')

    def addTable(self,new_image,step,title,all_message_dic,part_message_list): # 用于在主窗口界面添加页面
        # new_image是numpy图片
        # step表示进度条的进程
        # title表示页面的标题
    ...
    def autoJoint_2A(self):
        className_list = []
        dic = {} #key:窗口位置+场景名 value:属于该场景某一通道的图片列表
        for i in range(self.root_2A.childCount()):
            text = self.root_2A.child(i).text(0)
            string = text[22:24]+text[30:36]+ text[-10:-5]  # 窗口位置+场景名+序列号
            if not className_list.__contains__(string):
                className_list.append(string)
        ...
        try:
            self.joint_2AThread = Joint_2AThread(self.fileNames_2A,self.paths_2A,dic,className_list)
            self.processDiaog = ProcessDialog()
            self.joint_2AThread.start()
            self.joint_2AThread.finish.connect(self.addTable)
            if  self.joint_2AThread.exitcode == 1:
                raise self.joint_2AThread.exception
            self.processDiaog.exec_()
        except Exception:
            QMessageBox.information(self, "提示：", '操作失败!')

    def autoJoint_2B(self):
        className_list = []
        dic = {}  # key:窗口位置+场景名+序列号 value:属于该场景某一通道的文件名列表
        for i in range(self.root_2B.childCount()):
            text = self.root_2B.child(i).text(0)
            string = text[22:24] + text[30:36] + text[-10:-5]  # 窗口位置+场景名+序列号
            if not className_list.__contains__(string):
                className_list.append(string)
        for className in className_list:
        ...
        for i in range(self.root_2B.childCount()):
            text = self.root_2B.child(i).text(0)
            string = text[22:24] + text[30:36]+ text[-10:-5]
            dic[string].append(text)

        try:
            self.joint_2BThread = Joint_2BThread(self.fileNames_2B, self.paths_2B, dic, className_list)
            self.processDiaog = ProcessDialog()
            self.joint_2BThread.start()
            self.joint_2BThread.finish.connect(self.addTable)
            if self.joint_2BThread.exitcode == 1:
                raise self.joint_2BThread.exception
    ...
    def joint_2A(self):
        ...
        if jointWidget.exec_():
            ...
            newImage = np.zeros((jointImage.shape[0],jointImage.shape[1],1),dtype="uint16")
