...
class Add_ImageWidget(QWidget):
    ...
    def contextMenuEvent(self, event):
        ...
        register = contextMenu.addAction('几何校正')
        absoluteRadiate = contextMenu.addAction('绝对辐射校正')
        ...
        absoluteRadiate.triggered.connect(self.Event)
    ...
    def Event(self):
        ...
        if text == '详细信息':
            ...
            MessageShow = {}
            try:
                if len(self.part_detalMessage)!=0:
                    MessageShow.update(self.part_detalMessage[self.count])
                MessageShow.update(self.all_detalMessage)
                for k,v in MessageShow.items():
                    self.parent.message.append(str(k) + ":" + str(v))
            except:
                self.parent.message.append("无通道信息")

        if text == '保存当前通道':
            dict2 = {}
            try:
                dict2.update({"source":self.all_detalMessage["product_id"],"band":str(self.count+1)})
            except Exception as e:
                print(e)
            finally:
        ...
        if text == '填充':
            try:
                ...
                if expandEdit.exec_():
                    ...
                    new_image = np.zeros((height, width), dtype=dataType)
                    try:
                        new_image[pos_y:pos_y + image.shape[0], pos_x:pos_x + image.shape[1]] = image
                        self.setImage(new_image,None,None)
                        self.parentWidget().resize(width + 30, height + 60) #改变QMdiSubWindow的大小
                    except Exception as e:
                        self.parent.messageLog.append("填充失败！")
            except:
        ...
        if text == '裁剪':
            ...
            if shearEdit.exec_():
                ...
                height_2 = int(shearEdit.lineEdit_4.text())
                dataType = type(image.flatten()[0])
                new_image = np.zeros((height_2-height_1,width_2-width_1),dtype=dataType)
                try:
        ...
        if text == '合成彩色图像':
            ...
            if mergeWidget.exec_():
                ...
                dataType = type(self.imWidget.np_image.flatten()[0])
                images = np.zeros((self.imWidget.np_image.shape[0], self.imWidget.np_image.shape[1], len(fileNames_order)), dataType)
                ...
                sub.resize(images.shape[1], images.shape[0])
                ...
                imageWidget.setImage(images.copy(),None,None,True)
        ...
        if text == '画光谱曲线':
            ...
            ys = []
            band = 1
            normalImage = self.imWidget.np_image
            if len(points)==0:QMessageBox.information(self, "提示：", '未选择标记点！')
                ...
                for point in points:
                    ...
                    if len(self.imWidget.np_image.shape)>2:
                        ...
                        y = []
                        y.append(normalImage[int(real_y), int(real_x)])
                        ys.append(y)
        ...
        if text == "几何校正":
            if self.all_detalMessage is not None and self.all_detalMessage.keys().__contains__("register"):
                if self.all_detalMessage["register"] == "True":
                    QMessageBox.information(self.parent, "提示：", '无需再校正！')
                    return

            if not os.path.exists(MyUtils.translateTform_addr):
                paraWidget = ParaWidget()
                if paraWidget.exec_():
                    MyUtils.translateTform_addr = paraWidget.lineEdit.text()
            else:
                try:
                    new_image3D = MyUtils.geoCorrect(self.image3D,MyUtils.translateTform_addr,self.part_detalMessage,mode=32)
                    tabArea = QMdiArea()  # 添加页面窗口
                    sub = QMdiSubWindow()
                    sub.setWindowIcon(QIcon(os.path.join(LAST_DIR,"icon.png")))
                    sub.setWindowFlags(Qt.Tool)
                    ...
                    add_imageWidget.setWindowTitle("几何校正")
                    self.parent.messageLog.append("几何校正")
        ...
        if text == "绝对辐射校正":
            if self.all_detalMessage is not None and self.all_detalMessage.keys().__contains__("absoluteRadiateCorrect"):
                if self.all_detalMessage["absoluteRadiateCorrect"] == "True":
                    QMessageBox.information(self.parent, "提示：", '无需再校正！')
                    return
            try:
                new_image3D = np.zeros(self.image3D.shape, dtype="float64")
                for j in range(self.image3D.shape[2]):
                    calibration_parameter = self.part_detalMessage[j][
                        "absolute_radiation_calibration_parameter(W*m**-2*sr**-1*DN**-1)"]
                    new_image3D[:, :, j] = self.image3D[:, :, j] * float(calibration_parameter)
                tabArea = QMdiArea()  # 添加页面窗口
                sub = QMdiSubWindow()
                sub.setWindowIcon(QIcon(os.path.join(LAST_DIR, "icon.png")))
                sub.setWindowFlags(Qt.Tool)
                ...
                add_imageWidget = Add_ImageWidget(new_image3D, self.parent)
                all_detalMessage = self.all_detalMessage.copy()
                all_detalMessage.update({"absoluteRadiateCorrect": "True"})
                add_imageWidget.all_detalMessage = all_detalMessage
                add_imageWidget.part_detalMessage = self.part_detalMessage.copy()
                add_imageWidget.setObjectName("add_imageWidget")
                add_imageWidget.setWindowTitle("绝对辐射校正")
                self.parent.messageLog.append("绝对辐射校正")
                sub.setWidget(add_imageWidget)
                tabArea.addSubWindow(sub)
                ...
                QMessageBox.information(self.parent, "提示：", '缺少绝度辐射校正参数！')
                self.parent.messageLog.append("错误提示：" + str(e))
